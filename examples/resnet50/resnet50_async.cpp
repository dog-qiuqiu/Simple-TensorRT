...
int main(int argc, char** argv) {
    if (argc != 4) {
        ...
        std::cout << "\t" << "Usage: " << argv[0] << " <engine_path>" << " <image1_path> <image2_path>" << std::endl;
        return -1;
    }
    ...
    //输入预处理
    std::map<std::string, st::Tensor> inputs_tensor1;
    ret = InputProcess(image1, inputs_shape, inputs_tensor1);
    if (ret != 0) {
    ...
    //输入预处理
    std::map<std::string, st::Tensor> inputs_tensor2;
    ret = InputProcess(image2, inputs_shape, inputs_tensor2);
    if (ret != 0) {
        std::cout << "\033[31m" << "Input process failed" << "\033[0m" << std::endl;
        return -1;
    }
    ...
    for (int i = 0; i < 10; i++) {
        ...
        //推理1
        const int stream1 = 0;
        std::map<std::string, st::Tensor> outputs_tensor1;
        ret = tensorrt.ForwardAsync(inputs_tensor1, stream1);
        if (ret != 0) {
        ...
        //推理2
        const int stream2 = 1;
        std::map<std::string, st::Tensor> outputs_tensor2;
        ret = tensorrt.ForwardAsync(inputs_tensor2, stream2);
        if (ret != 0) {
            std::cout << "\033[31m" << "TensorRT forward failed" << "\033[0m" << std::endl;
            return -1;
        }
        ...
        //获取推理1结果
        ret = tensorrt.GetForwardAsyncOutput(outputs_tensor1, stream1);
        if (ret != 0) {
            std::cout << "\033[31m" << "Get forward output failed" << "\033[0m" << std::endl;
            return -1;
        }

        //输出1后处理
        std::vector<std::pair<int, float>> result1;
        ret = OutputProcess(outputs_tensor1, result1);
        if (ret != 0) {
        ...
        for (int i = 0; i < result1.size(); i++) {
            std::cout << "image1: " << "batch: " << i << " class: " << result1[i].first << " score: " << result1[i].second << std::endl;
        }

        //获取推理2结果
        ret = tensorrt.GetForwardAsyncOutput(outputs_tensor2, stream2);
        if (ret != 0) {
            std::cout << "\033[31m" << "Get forward output failed" << "\033[0m" << std::endl;
            return -1;
        }
        ...
        ret = OutputProcess(outputs_tensor2, result2);
        if (ret != 0) {
            std::cout << "\033[31m" << "Output process failed" << "\033[0m" << std::endl;
            return -1;
        }
        ...
        for (int i = 0; i < result2.size(); i++) {
            std::cout << "image2: " << "batch: " << i << " class: " << result2[i].first << " score: " << result2[i].second << std::endl;
        }
    }
    //销毁TensorRT资源
